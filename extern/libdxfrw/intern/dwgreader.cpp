...
bool dwgReader::readDwgBlocks(DRW_Interface& intfa, dwgBuffer *dbuf){
    ...
    for (std::map<duint32, DRW_Block_Record*>::iterator it=blockRecordmap.begin(); it != blockRecordmap.end(); ++it){
        ...
        //in dwg code 330 are not set like dxf in ModelLayout & PaperLayout, set it (RLZ: only tested in 2000)
        if (bk.parentHandle == static_cast<int>(DRW::HandleCodes::NoHandle)) {
            //in dwg code 330 are not set like dxf in ModelLayout & PaperLayout, set it
        ...
        delete[]tmpByteStr;
        ret = ret && ret2;
        if (bk.parentHandle == static_cast<int>(DRW::HandleCodes::NoHandle)) bk.parentHandle= bkr->handle;
        parseAttribs(&end);
        intfa.endBlock();
    }
...
#define ENTRY_PARSE(e) \
    ...
    nextEntLink = prevEntLink = 0;//set to 0 to skip unimplemented entities
        ...
        case 8: {//minsert = 8
            DRW_Insert e;
            ENTRY_PARSE(e)
            e.name = findTableName(DRW::TTYPE::BLOCK_RECORD, e.blockRecH.ref);//RLZ: find as block or blockrecord (ps & ps0)
            intfa.addInsert(e);
            break; }
        case 77: {
            DRW_LWPolyline e;
        ...
        case 1: {
            DRW_Text e;
            ENTRY_PARSE(e)
            e.style = findTableName(DRW::TTYPE::STYLE, e.styleH.ref);
            intfa.addText(e);
            break; }
        case 44: {
            DRW_MText e;
        ...
        case 20: {
            DRW_DimOrdinate e;
            ENTRY_PARSE(e)
            e.style = findTableName(DRW::TTYPE::DIMSTYLE, e.dimStyleH.ref);
...
//   if (oType == 68 && version== DRW::AC1015){//V2000 dimstyle seems have one unknown byte hard handle counter??
    if (oType == 68 && version > DRW::Version::AC1014){//dimstyle seems have one unknown byte hard handle counter??
        unkData = buf->getRawChar8();
        DRW_DBG(" unknown v2000 byte: "); DRW_DBG( unkData); DRW_DBG("\n");
    }
    if (version > DRW::Version::AC1018){//from v2007+ have a bit for strings follows (ObjControl do not have)
