...
//   writer = NULL;
    applyExt = false;
    version = DRW::Version::UNKNOWNV;
    error = DRW::error::BAD_NONE;
}
...
bool dwgR::getPreview(){
    ...
    if (isOk) {
        ...
        error = DRW::error::BAD_READ_METADATA;
...
//testReader();return false;
    ...
    if (isOk) {
        ...
        if (isOk) {
            ...
            error = DRW::error::BAD_READ_FILE_HEADER;
    } else
        error = DRW::error::BAD_READ_METADATA;
...
bool dwgR::processDwg() {
    ...
    if (!ret) {
        error = DRW::error::BAD_READ_HEADER;
    }

    ret2 = reader->readDwgClasses();
    if (ret && !ret2) {
        error = DRW::error::BAD_READ_CLASSES;
        ret = ret2;
    }

    ret2 = reader->readDwgHandles();
    if (ret && !ret2) {
        error = DRW::error::BAD_READ_HANDLES;
        ret = ret2;
    }

    ret2 = reader->readDwgTables(hdr);
    if (ret && !ret2) {
        error = DRW::error::BAD_READ_TABLES;
        ret = ret2;
    }
    ...
    if (ret && !ret2) {
        error = DRW::error::BAD_READ_BLOCKS;
        ret = ret2;
    }

    ret2 = reader->readDwgEntities(*iface);
    if (ret && !ret2) {
        error = DRW::error::BAD_READ_ENTITIES;
        ret = ret2;
    }

    ret2 = reader->readDwgObjects(*iface);
    if (ret && !ret2) {
        error = DRW::error::BAD_READ_OBJECTS;
        ret = ret2;
    }
