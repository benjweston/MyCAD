...
public:
    //initializes default values
        ...
        //line weight: default BYLAYER  (dxf -1, dwg 29), space: default ModelLayout (0)
    DRW_Entity(): eType(DRW::ETYPE::UNKNOWN), handle(static_cast<int>(DRW::HandleCodes::NoHandle)), parentHandle(static_cast<int>(DRW::HandleCodes::NoHandle)), appData(0),
                  space(DRW::Space::ModelLayout), layer("0"), lineType("BYLAYER"), material(static_cast<int>(DRW::MaterialCodes::MaterialByLayer)),
                  color(static_cast<int>(DRW::ColorCodes::ColorByLayer)), lWeight(DRW_LW_Conv::lineWidth::widthByLayer), ltypeScale(1.0), visible(true),
                  numProxyGraph(0), proxyGraphics(std::string()), color24(-1), colorName(std::string()),
                  transparency(static_cast<int>(DRW::TransparencyCodes::Opaque)), plotStyle(static_cast<int>(DRW::PlotStyleCodes::DefaultPlotStyle)), shadow(DRW::ShadowMode::CastAndReceieveShadows),
...
protected:
    ...
    //parses dwg cmdson start part to read entity
    bool parseDwg(DRW::Version version, dwgBuffer *buf, dwgBuffer* strBuf, duint32 bs=0);
    //parses dwg cmdson handles part to read entity
...
public:
	...
	DRW_Coord BasePoint() { return basePoint; }
...
public:
    DRW_Line() {
        eType = DRW::ETYPE::LINE;
        secPoint.z = 0;
    }

    virtual void applyExtrusion(){}

	DRW_Coord SecondPoint() { return secPoint; }
...
class DRW_Circle : public DRW_Point {
    SETENTFRIENDS
public:
    DRW_Circle() : radious(0.0) {
        eType = DRW::ETYPE::CIRCLE;
    }

    virtual void applyExtrusion();

	//! centre point in OCS
	const DRW_Coord &Centre() { return basePoint; }
...
class DRW_Arc : public DRW_Circle {
    SETENTFRIENDS
public:
    DRW_Arc() : staangle(0.0) , endangle(0.0) {
        eType = DRW::ETYPE::ARC;
        isccw = 1;
    }
...
class DRW_Ellipse : public DRW_Line {
    SETENTFRIENDS
public:
    DRW_Ellipse() : ratio(0.0), staparam(0.0), endparam(0.0) {
        eType = DRW::ETYPE::ELLIPSE;
        isccw = 1;
    }
...
class DRW_LWPolyline : public DRW_Entity {
    SETENTFRIENDS
public:
    DRW_LWPolyline() : vertexnum(0) {
        eType = DRW::ETYPE::LWPOLYLINE;
    ...
    DRW_LWPolyline(const DRW_LWPolyline& p):DRW_Entity(p){
        this->eType = DRW::ETYPE::LWPOLYLINE;
...
class DRW_Vertex : public DRW_Point {
    SETENTFRIENDS
public:
    DRW_Vertex() : tgdir(0.0) {
        eType = DRW::ETYPE::VERTEX;
        stawidth = endwidth = bulge = 0;
        vindex1 = vindex2 = vindex3 = vindex4 = 0;
        flags = identifier = 0;
    }
    DRW_Vertex(double sx, double sy, double sz, double b) : tgdir(0.0) {
        stawidth = endwidth = 0;
        vindex1 = vindex2 = vindex3 = vindex4 = 0;
        flags = identifier = 0;
        basePoint.x = sx;
...
class DRW_Polyline : public DRW_Point {
    SETENTFRIENDS
public:
    DRW_Polyline() : firstEH(0), lastEH(0) {
        eType = DRW::ETYPE::POLYLINE;
...
public:
    ...
    int smoothN;				/*!< smooth surface N density, code 74, default 0 */
...
class DRW_Spline : public DRW_Entity {
    SETENTFRIENDS
public:
    DRW_Spline() : controlpoint(0), fitpoint(0), degree(0) {
        eType = DRW::ETYPE::SPLINE;
...
class DRW_Image : public DRW_Line {
    SETENTFRIENDS
public:
    DRW_Image() : dz(0.0), ref(0), sizeu(0.0), sizev(0.0) {
        eType = DRW::ETYPE::IMAGE;
        fade = clip = 0;
...
public:
    DRW_DimOrdinate() {
        eType = DRW::ETYPE::DIMORDINATE;
    }
    DRW_DimOrdinate(const DRW_Dimension& d): DRW_Dimension(d) {
        eType = DRW::ETYPE::DIMORDINATE;
    }

    DRW_Coord getOriginPoint() const {return getDefPoint();}   /*!< m_bOrigin definition point, code 10, 20 & 30 */
...
class DRW_Leader : public DRW_Entity {
    SETENTFRIENDS
public:
    DRW_Leader() : annotHandle(0), coloruse(0), hookline(0), textheight(0.0), textwidth(0.0), vertexpoint(0) {
        eType = DRW::ETYPE::LEADER;
        flag = 3;
...
class DRW_Viewport : public DRW_Point {
    SETENTFRIENDS
public:
    DRW_Viewport() : backClip(0.0), frontClip(0.0), frozenLyCount(0), snapAngle(0.0), snapPX(0.0), snapPY(0.0),
        snapSpPX(0.0), snapSpPY(0.0), twistAngle(0.0), viewHeight(0.0), viewLength(0.0), vpID(0) {
        eType = DRW::ETYPE::VIEWPORT;
        vpstatus = 0;
        pswidth = 205;
        psheight = 156;
        centerPX = 128.5;
